...
| "Point the coding agent you already use -- Claude Code, Codex, OpenClaw, Hermes, Trae -- at it." | The core is markdown plus shell-based Node/Python substrate. Claude Code had an earlier conformance receipt; Codex has a local conformance receipt after this audit. OpenClaw, Hermes, and Trae are not independently driven in this repo. | Partially proven | "Point Claude Code, Codex, or any coding agent that can read markdown and run shell commands at it. OpenClaw, Hermes, Trae, and others should run the included conformance probe before you claim them as verified." |
