...
function agentInstructionsBlock(hostIds: string[]): string {
  return [
    ...
    "- Direction-changing input must run `sfn direction intake/propose/decide/apply` before architecture or product code edits continue.",
...
function sfnHookRuntimeCommon(): string[] {
  return [
    ...
    "}",
    "function directionRequiredMissing() {",
    "  try {",
    "    const receipt = resolve('.solo/receipts/R/direction-change-receipt.json');",
    "    if (existsSync(receipt)) return false;",
    "    const events = resolve('.solo/events.jsonl');",
    "    if (!existsSync(events)) return false;",
    "    const log = readFileSync(events, 'utf8');",
    "    return /(change direction|instead of|not integrate|make our own|oss|open[- ]source|learn from|gold standard|new architecture|new pipeline|replace|pivot|rethink|spline|emergent|bruno simon|igloo|architecture governor|research governor|proof registry)/i.test(log);",
    "  } catch {",
    "    return false;",
    "  }",
    "}",
    "function isCodeWriteCommand(command) {",
    "  return /(apply_patch|file\\.write|Set-Content|Add-Content|Out-File|git\\s+commit|npm\\s+run\\s+build|pnpm\\s+build|yarn\\s+build|vercel\\s+--prod)/i.test(command);",
    "}",
    "function isDirectionProtocolCommand(command) {",
    "  return /sfn\\s+--\\s+direction|sfn\\s+direction|direction\\s+(intake|propose|decide|apply)/i.test(command);",
    "}",
  ];
}
...
function sfnPreToolPolicyScript(): string {
  return [
    ...
    "  process.exit(1);",
    "}",
    "if (directionRequiredMissing() && isCodeWriteCommand(command) && !isDirectionProtocolCommand(command) && process.env.SFN_ALLOW_DIRECTIONLESS_WRITE !== '1') {",
    "  record('tool.error', ['--status', 'blocked', '--message', 'blocked code write before direction-change receipt']);",
    "  console.error(JSON.stringify({ decision: 'block', reason: 'Direction-changing input is pending. Run sfn direction intake/propose/decide/apply and update research/system-map/proof receipts before code writes.', command }));",
    "  process.exit(1);",
    "}",
